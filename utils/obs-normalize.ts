...
export type ObsSectionIntro = {
  type: 'intro';
  text: string;
};

export type ObsSectionPoint = {
  type: 'point';
  number: number;
  title: string;
  answer: string | null;
...
export type ObsSectionApplication = {
  type: 'application';
  items: ObsTreeNode[];
};
...
function fillBlank(text: string, answer?: unknown): string {
  ...
  if (!text) return '';
  return answerText ? text.replace(/\(\s*\)/g, `(${answerText})`) : text;
}
...
function normalizeTreeNodes(rawItems: unknown[], fallbackAnswer: string | null = null): ObsTreeNode[] {
  return rawItems
    .map<ObsTreeNode | null>((item) => {
      if (!item || typeof item !== 'object') return null;

      const record = item as RawRecord;
      const rawText = asText(record.text);
      if (!rawText) return null;

      const ownAnswer = asText(record.answer) || null;
      const answer = ownAnswer ?? (fallbackAnswer && hasBlank(rawText) ? fallbackAnswer : null);
      const children = Array.isArray(record.children)
        ? normalizeTreeNodes(record.children, fallbackAnswer)
        : [];
      const notes = Array.isArray(record.notes)
        ? record.notes.map(asText).filter(Boolean).map((note) => fillBlank(note, answer ?? fallbackAnswer))
        : [];

      return {
        number: asText(record.number),
        text: fillBlank(rawText, answer),
        answer,
        reference: asText(record.reference) || null,
        numbered: typeof record.numbered === 'boolean' ? record.numbered : true,
        children,
        notes,
      };
    })
    .filter((item): item is ObsTreeNode => item !== null);
}
...
function normalizeApplication(section: RawRecord): ObsSection {
  ...
  const normalizedItems =
    existingItems.length > 0 && typeof existingItems[0] === 'object' && existingItems[0] && 'children' in (existingItems[0] as RawRecord)
      ? normalizeTreeNodes(existingItems)
...
export function normalizeObsSections(rawSections: unknown): ObsSection[] {
  ...
  source.forEach((section) => {
    ...
    const title = fillBlank(asText(record.title), answer);
    const items =
      ...
      'children' in (record.items[0] as RawRecord)
        ? normalizeTreeNodes(record.items, answer)
